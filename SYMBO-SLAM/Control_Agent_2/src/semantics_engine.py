...
class semantics_engine:
    def __init__(self, m_p_global_map, class_name_map, homeloc):
        ...
        SymboSLAM_loc = homeloc + "ontology/SymboSLAM.owl"
    ...
    def transform_feat_map(self,m_p_global_map):
        try:
            ...
            message = "Error: Unable segment map - " + str(e)
            print(message)
    ...
    '''
    def context_reasoner(self, m_p_global_map, onto, feature_map, env_feats, env_prob):
        try:
            ...
            for key in segment_map_index:
                ...
                for item in extracted_feats:
                    ...
                    feat_name = (("*" + item.name).replace(" ", "_")).lower()
                ...
                for env in env_prob:
                    #calculate env liklihood using feature prob, semantic proximity and spatial proximity
    ...
    def semantics_engine_main(self, m_p_global_map, class_name_map, SymboSLAM, OntoSLAM, Onto4MAT ):
        try:
            ...
            for env in base_environments:
                cur_envs = list(env.subclasses())
                environments.extend(cur_envs)
            ...
            while True:
                ...
                self.context_reasoner(m_p_global_map, SymboSLAM, feature_map, env_feats, env_prob)
                OSWait(3000);
